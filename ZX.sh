...
# 核心功能：亮屏 - 智能休眠关闭/熄屏 - 智能休眠开启 + 低电量保护 + 充电智能休眠关 
[ ! "$MODDIR" ] && MODDIR=${0%/*}
...
main_loop() {
    while true; do
        ...
        charging=$(cat /sys/class/power_supply/battery/status 2>/dev/null | grep -i "charging\|full")
        if [ -n "$charging" ]; then
            ...
            echo "$(date '+%Y年%m月%d日%H时%M分%S秒') ⚡ 充电中 - 智能休眠关闭 - 性能" >> "$LOG_FILE"
        else
            if [ "$ba" -lt 20 ]; then
                ...
                echo "$(date '+%Y年%m月%d日%H时%M分%S秒') 🔋 电量<20% 强制智能休眠开启 - 省电" >> "$LOG_FILE"
            else
